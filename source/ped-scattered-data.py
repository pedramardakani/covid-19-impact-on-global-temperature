...
def getdata(index,check_index=0):
    # check which file to read
    if index < 18:
        f   = nio.open_file(filename,'r')
        # get the data ready
        var = f.variables['skt'][index,0,check_index,:,:]
    else:
        f   = nio.open_file(filename_new,'r')
        # get the data ready
        var = f.variables['skt'][18 - index,check_index,:,:]
...
filename = "/media/pedram/dataStorage/Ped_Doc/Research/myProjects/saeed-shojaei/pyngl/adaptor.mars.internal-1590162992.3450093-30030-25-7410f2f4-da91-4ef7-bf96-c68cda090a23.nc"
filename_new =  "/media/pedram/dataStorage/Ped_Doc/Research/myProjects/saeed-shojaei/pyngl/adaptor.mars.internal-1590157005.8169813-22157-7-4452e7e0-a81b-433d-ad07-25b18e498e86.nc"

#-- set saved file name
plotname = "manual-scale-scattered-data-index-"+str(check_index)+"-"
#plottype = "png"
plottype = "pdf"
...
if t_i == None:
    var = getdata(t_f,check_index)
else:
    var = getdata(t_f,check_index) - getdata(t_i,check_index)
...
#-- check if the data is in a difference or not
if t_i == None:
    ngl.text_ndc(wks, plotname + '(C). Date: ' + str(gettime(t_f)), 0.50,0.82,txres)
else:
